...
<template>
  <div class="citizens-view">
    ...
    <div class="citizens-list">
      <div v-for="citizen in citizens" :key="citizen._id" class="citizen-item">
        <div
          class="citizen-name"
        ...
        >
          ...
          >
            <div class="tooltip-content">
              {{
                getCitizenCityInfo(citizen)
                  ? `${getCitizenCityInfo(citizen).name}, ${getCitizenCityInfo(citizen).population} жителей`
                  : 'Город не указан'
              }}
            </div>
          </div>
        ...
        <div class="citizen-hierarchy">
          ...
          >
            <span class="level-type">{{
              getLevelDisplayName(group.type)
            }}</span>
            {{ group.name }}
            <span v-if="index < citizen.groups.length - 1" class="arrow"
              >→</span
            >
          </span>
        </div>
