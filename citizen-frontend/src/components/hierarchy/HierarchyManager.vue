<script setup>
  ...
  function saveChanges() {
    ...
    const incompleteLevel = localHierarchyConfig.value.find(
      (level) => !level.type.trim() || !level.name.trim(),
    )
    if (incompleteLevel) {
      alert(
        'Ошибка: Поля "Тип уровня" и "Название уровня" должны быть заполнены для всех уровней!',
      )
      return
    }
  ...
  function validateTypeUniqueness() {
    const types = localHierarchyConfig.value
